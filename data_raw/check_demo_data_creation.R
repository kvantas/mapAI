...
assessment_rf_random <- assess_pai_model(gcp_data, pai_method = "rf", validation_type = "random", k_folds = 5)
assessment_rf_spatial <- assess_pai_model(gcp_data, pai_method = "rf", validation_type = "spatial", k_folds = 5)

# Assess gam with both random and spatial CV
assessment_gam_random <- assess_pai_model(gcp_data, pai_method = "gam", validation_type = "random", k_folds = 5)
assessment_gam_spatial <- assess_pai_model(gcp_data, pai_method = "gam", validation_type = "spatial", k_folds = 5)

# Assess Linear Model with both random and spatial CV
assessment_lm_random <- assess_pai_model(gcp_data, pai_method = "lm", validation_type = "random", k_folds = 5)
assessment_lm_spatial <- assess_pai_model(gcp_data, pai_method = "lm", validation_type = "spatial", k_folds = 5)
