...
# models (i.e. lm, tps, tin_akima) and the second about bivariate models (i.e.
# gam_biv, helmert, tin_linear, hybrid_helmert_tin, hybrid_affine_tin).
#
...
# Define Model 3: Akima C1 Triangulated Spline (tin_akima)
tin_akima_model <- list(
  ...
  predict = function(modelFit, newdata, ...) {
    ...
    na_idx <- is.na(preds)
    if (any(na_idx)) {
      preds[na_idx] <- stats::predict(modelFit$fallback_fit, newdata = newdata[na_idx, , drop = FALSE])
    }
    return(preds)
  }
)
...
# Define Model 6: Piecewise Affine Delaunay Triangulation (tin_linear)
tin_linear_model <- list(
  ...
  fit = function(dat, fallback = c("helmert", "affine"), ...) {
    ...
    # Inversion analysis on triangles
...
# Define Model 7: Hybrid Helmert-TIN Model (hybrid_helmert_tin)
...
# Define Model 8: Hybrid Affine-TIN Model (hybrid_affine_tin)
