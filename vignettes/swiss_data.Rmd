...
Before training a final model, it is crucial to assess its expected performance. The `cv_pai_model` function provides a robust framework for this. We will compare four different validation strategies to understand how the model might perform on new data.
  ...
  * **Design-Based Validation**: We also test validation based on sampling probability (`probability`) and k-fold stratified sampling (`stratified`), which are useful when the data points have a known design or structure.


```{r assess_model, message=FALSE}
# Run four different validation methods using a GAM
cvra <- cv_pai_model(swiss_cps, pai_method = "gam_biv", validation_type = "random", k_folds = 10, seed = 1)
cvsp <- cv_pai_model(swiss_cps, pai_method = "gam_biv", validation_type = "spatial", k_folds = 10, seed = 1)
valr <- cv_pai_model(swiss_cps, pai_method = "gam_biv", validation_type = "probability", seed = 1)
vals <- cv_pai_model(swiss_cps, pai_method = "gam_biv", validation_type = "stratified", seed = 1)

# Combine and format the results into a table
validation_df <- rbind(cvra$summary, cvsp$summary, valr$summary, vals$summary) %>%
...
final_gam_model <- train_pai_model(swiss_cps, "gam_biv")
...
gam_corrected_grid <- transform_map(final_gam_model, source_grid)
