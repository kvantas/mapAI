...
By offering this comprehensive suite of methods (random CV, SCV, and design-based validation), you can move beyond simply comparing traditional CV strategies to perform a more robust assessment of your model's predictive accuracy.

Let's compare the results from these methods for all three models (`lm`, `rf`, and `gam`).

```{r validate models, message=FALSE}
...
stratified_results <- validate_all_methods(gcps, "stratified")
...
pai_model <- train_pai_model(gcps, method = "lm")
