...
The same trained PAI model can be applied directly to correct raster maps using `apply_pai_raster()`. Because raster data sits on a regular grid of pixels, `apply_pai_raster()` uses inverse mapping and resampling to compute pixel values without leaving holes or artifacts.
