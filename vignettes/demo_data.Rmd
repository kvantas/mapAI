...
vignette: >
  %\VignetteIndexEntry{Getting Started with mapAI}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/vignette1-",
  out.width = "100%"
)
```
...
We calculate the initial 2D Root Mean Squared Error (RMSE) to get a baseline for the positional error. Then, we train a **Generalized Additive Model (`gam`)**, which is well-suited for capturing the smooth, non-linear distortions in our synthetic data.
...
```{r apply-and-visualize, fig.dim = c(6, 6)}
...
```{r plot-surfaces, fig.dim = c(8, 4), out.width="100%"}
...
```{r plot-indicatrices, fig.dim = c(6, 6)}
