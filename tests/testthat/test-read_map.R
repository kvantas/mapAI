test_that("read_map() works and handles errors", {
  ...
  map_data <- create_dummy_shp(file.path(temp_dir, "test_map.shp"))

  map_data <- read_map(shp_path = map_data)
...
test_that("read_map() calculates 'area_old' for polygons", {
  ...
  temp_dir <- withr::local_tempdir()
  map_data <- create_dummy_shp(file.path(temp_dir, "test_map.shp"), has_area_old = TRUE)

  poly_map <- read_map(map_data)

  expect_true("area_old" %in% names(poly_map))
  expect_equal(as.numeric(poly_map$area_old[1]), 1)
...
test_that("read_map does not calculate area_old if it already exists", {
  ...
  expect_equal(as.numeric(map$area_old), 1) # Check the value set in create_dummy_shp
