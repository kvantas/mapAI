...
#'       Read using `terra::rast()`, returning an in-memory `terra::SpatRaster` object.
#'   }
#'
...
#' # vector_map <- read_map("path/to/map.shp", quiet = TRUE)
#'
#' # Read the raster map that needs correction
#' # raster_map <- read_map("path/to/raster.tif")
#' }
