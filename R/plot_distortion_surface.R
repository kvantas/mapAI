...
#' @description Creates a visualization of a continuous distortion metric from
...
#'   rasterized surface plot using `geom_raster()`, which is ideal for
#'   visualizing continuous distortion surfaces. To create a regular grid,
#'   use the pattern: `sf::st_make_grid(...) %>% sf::st_centroid()
#'   %>% sf::st_sf()`.
...
#' `distortion_sf` data:
#' \itemize{
#'   \item **`gam` or `tps` model data**: Will produce a smooth, continuous
...
#'     transformation (`area_scale` = 1, `max_shear` = 0). While RF might be an
...
#'   `analyze_distortion()`.
#' @param metric A character string specifying the name of the metric column to
#'   plot. Must be one of "a", "b", "area_scale", "log2_area_scale",
#'   "max_shear", "max_angular_distortion", "airy_kavrayskiy", or "theta_a".
#' @param gcp_data An optional `sf` object of the original GCPs to overlay on
#'   the plot for context.
#' @param palette A character string specifying the name of a viridis color
...
plot_distortion_surface <- function(distortion_sf, metric, gcp_data = NULL,
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::labs(title = paste("Distortion Analysis:", metric),
  ...
  if (is_grid) {
    ...
    p <- p +
      ggplot2::geom_raster(
        data = plot_df, aes(x = .data$X, y = .data$Y, fill = .data$metric_val))
    ...
    p <- p +
      ggplot2::geom_point(
        data = plot_df,
        aes(x = .data$X, y = .data$Y, color = .data$metric_val),
        size = 3)
