...
#'     \item \strong{`spatial`}: Spatial k-fold cross-validation using k-means clustering.
#'     \item \strong{`probability`}: A single train/test split using simple random sampling.
...
#' @param pai_method A character string or a custom model list for `train_pai_model`.
#' @param validation_type The validation strategy: "random", "spatial", "probability", or "stratified".
#' @param k_folds Number of folds for CV. Defaults to 5.
#' @param train_split_ratio Proportion of data for training (for "probability" and "stratified"). Defaults to 0.8.
...
cv_pai_model <- function(gcp_data, pai_method,
  ...
  all_predictions <- lapply(seq_along(splits), function(i) {
    ...
    if (length(splits) > 1) message(paste("  Processing Fold", i, "of", length(splits), "..."))

    model <- train_pai_model(gcp_data = train_data, method = pai_method, seed = seed + i, ...)
  ...
  if (validation_type %in% c("random", "spatial", "stratified")) {
    ...
    fold_rmse <- vapply(split(all_predictions_df, all_predictions_df$fold), function(df) {
    ...
    mean_rmse <- sqrt(mean((all_predictions_df$true_dx - all_predictions_df$pred_dx)^2 +
...
create_resampling_splits <- function(gcp_data, type, k, ratio, n_strata) {
  ...
  switch(type,
         "random" = {
           ...
           lapply(1:k, function(i) list(train = which(fold_ids != i), test = which(fold_ids == i)))
         },
         "spatial" = {
         ...
         "probability" = {
           ...
           list(list(train = train_indices, test = setdiff(indices, train_indices)))
         },
         "stratified" = {
           dist <- sqrt(gcp_data$dx^2 + gcp_data$dy^2)
           breaks <- stats::quantile(dist, probs = seq(0, 1, by = 1/n_strata), na.rm = TRUE, names = FALSE)
           breaks <- unique(breaks)

           if (length(breaks) < 2) {
             warning("Could not create strata; falling back to simple random sampling.", call. = FALSE)
             return(create_resampling_splits(gcp_data, "probability", k, ratio, n_strata))
           }

           strata_ids <- cut(dist, breaks = breaks, include.lowest = TRUE, labels = FALSE)

           lapply(1:n_strata, function(i) {
             list(train = which(strata_ids != i), test = which(strata_ids == i))
             })
         }
  )
}
...
#' @description Provides a concise and formatted summary of the model assessment results.
...
print.pai_assessment <- function(x, ...) {
  ...
  if (x$summary$ValidationType %in% c("random", "spatial")) {
    ...
    cat("Train/Test Split:  ", paste0(x$details$train_split_ratio * 100, "% / ",
  ...
  if (!is.na(x$summary$SD_RMSE_2D)) {
    cat(sprintf("Std Dev of RMSE:    %.4f (across folds)\n", x$summary$SD_RMSE_2D))
  }
